...
##### \_\_name\_\_属性  
一个模块被另一个程序第一次引入时，其主程序将运行，如果想在模块被引入时，模块中的某依程序快不执行，可以用\_\_name\_\_属性来使该模块自身运行时执行。  
```
a.py  
if __name__ == '__main'__:
  print('程序自身在运行')  //如果__name__==a,则执行这句
else:
  print('我来自另一模块')
...
##### 搜索路径  
![](https://github.com/19969/python/blob/master/img/%E8%B7%AF%E5%BE%84.PNG)  
不同的系统显示的路径时不一样的，最佳存放模块的位置是C:\\Python34\\lib\\site-packages  
...
##### 包  
包是一种管理python模块命名空间的形式，一个模块的名称是A.B，那么它表示一个包A中子模块B。
创建包：  
  1.创建一个文件夹，存放相关模块，文件夹名即包名  
